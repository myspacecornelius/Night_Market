"""Add hyperlocal features

Revision ID: b23d7f450244
Revises: e3c11fbeb6ab
Create Date: 2025-08-20 04:43:05.274254
...
revision: str = 'b23d7f450244'
down_revision: Union[str, Sequence[str], None] = 'e3c11fbeb6ab'
...
def upgrade() -> None:
    ...
    sa.Column('transaction_type', sa.Enum('DAILY_STIPEND', 'BOOST_SENT', 'BOOST_RECEIVED', 'SIGNAL_REWARD', name='transaction_type_enum'), nullable=False),
    ...
    post_type_enum = postgresql.ENUM('SPOTTED', 'STOCK_CHECK', 'LINE_UPDATE', 'GENERAL', name='post_type_enum')
    post_type_enum.create(op.get_bind())
    op.add_column('posts', sa.Column('location_id', sa.UUID(), nullable=False))
    op.add_column('posts', sa.Column('post_type', sa.Enum('SPOTTED', 'STOCK_CHECK', 'LINE_UPDATE', 'GENERAL', name='post_type_enum'), nullable=False))
    op.add_column('posts', sa.Column('boost_score', sa.Integer(), nullable=True))
    op.create_foreign_key('fk_posts_location_id', 'posts', 'locations', ['location_id'], ['id'])
    op.drop_column('posts', 'geo_tag_lat')
    op.drop_column('posts', 'geo_tag_long')
    op.drop_column('posts', 'content_type')
    op.add_column('users', sa.Column('laces_balance', sa.Integer(), nullable=False))
...
def downgrade() -> None:
    ...
    op.add_column('posts', sa.Column('content_type', postgresql.ENUM('text', 'image', 'video', name='content_type_enum'), autoincrement=False, nullable=False))
    op.add_column('posts', sa.Column('geo_tag_long', sa.DOUBLE_PRECISION(precision=53), autoincrement=False, nullable=True))
    op.add_column('posts', sa.Column('geo_tag_lat', sa.DOUBLE_PRECISION(precision=53), autoincrement=False, nullable=True))
    op.drop_constraint('fk_posts_location_id', 'posts', type_='foreignkey')
    op.drop_column('posts', 'boost_score')
    ...
    op.create_index(op.f('countysub_lookup_name_idx'), 'countysub_lookup', [sa.text('soundex(name::text)')], unique=False)
    ...
    op.create_index(op.f('county_lookup_name_idx'), 'county_lookup', [sa.text('soundex(name::text)')], unique=False)
    ...
    op.create_index(op.f('place_lookup_name_idx'), 'place_lookup', [sa.text('soundex(name::text)')], unique=False)
    ...
    op.create_index(op.f('idx_tiger_featnames_snd_name'), 'featnames', [sa.text('soundex(name::text)')], unique=False)
    op.create_index(op.f('idx_tiger_featnames_lname'), 'featnames', [sa.text('lower(name::text)')], unique=False)
    ...
    op.drop_index('ix_locations_point', table_name='locations', postgresql_using='gist')
